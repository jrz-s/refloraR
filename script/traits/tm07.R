  ...
  df |>
    mutate(
      estrutura_principal = case_when(
        estrutura %in% c("sepala", "petala", "labelo", "coluna") ~ "flor",
        TRUE ~ estrutura
      )
    )
}
...
traits_quant <- segmentar_estruturas(text.test) |>
  passo_B_completo() |>
  mutate(
    dados_quant = purrr::map(segmento, extrair_quantitativos_segmento)
  ) |>
  tidyr::unnest(dados_quant) |>
  mutate(
...
extrair_qualitativos_segmento <- function(texto) {
  ...
  purrr::imap_dfr(dic_qualitativo, function(termos, categoria) {
    ...
    encontrados <- termos[stringr::str_detect(texto, termos)]
...
traits_qual <- segmentar_estruturas(text.test) |>
  passo_B_completo() |>
  mutate(
    dados_qual = purrr::map(segmento, extrair_qualitativos_segmento)
  ) |>
  tidyr::unnest(dados_qual) |>
  mutate(
...
# para que possam unir precisa ter as mesmas colunas
