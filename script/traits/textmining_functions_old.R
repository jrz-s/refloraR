...
#   
#   # ---------------------------------------------------------------
#   # Subfunção: normalizar dimensões bidimensionais
...
#       function(m) {
#         
#         nums <- stringr::str_match(m, padrao_2d)
#         
#         m1 <- mean(as.numeric(nums[2:3]))
#         m2 <- mean(as.numeric(nums[4:5]))
#         unit <- nums[6]
#         
#         paste0(round(m1, 2), "-", round(m2, 2), unit)
#       }
#     )
#   }
#   
#   # ---------------------------------------------------------------
#   # Subfunção: extrair contagens ontológicas
#   extrair_contagens <- function(texto) {
#     
#     padrao_contagem <- stringr::regex(
#       "(?:numero(?:\\s+de)?\\s*)?(\\d+(?:/\\d+)*)",
#       ignore_case = TRUE
#     )
#     
#     matches <- stringr::str_match_all(texto, padrao_contagem)[[1]]
#     
#     if (nrow(matches) == 0) {
#       return(dplyr::tibble())
#     }
...
#   
#   # ---------------------------------------------------------------
#   # 1. Dimensões (com tolerância semântica)
#   
#   padrao_dim <- stringr::regex(
#     "(\\d+(?:\\.\\d+)?)\\s*
#      (?:-|ate)?\\s*
...
#   )
#   
#   res_dim <- stringr::str_match_all(texto, padrao_dim)[[1]]
#   
#   tib_dim <- if (nrow(res_dim) == 0) {
#     
#     dplyr::tibble()
#     
#   } else {
...
#   # 2. Contagens
#   
#   tib_count <- extrair_contagens(texto)
#   
#   # ---------------------------------------------------------------
#   # Saída
#   
...
#       value = dplyr::if_else(is.na(max), min, (min + max) / 2),
#       trait_type = "quantitative",
#       trait = NA_character_ # ficar atento! add 09.01.2026
#     ) %>%
#     dplyr::select(trait_type, trait, min, max, value, unit)
# }
